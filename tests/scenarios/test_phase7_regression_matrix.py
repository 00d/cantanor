...
REGRESSION_DIR = Path("scenarios/regression_phase7")
EXPECTED_PATH = REGRESSION_DIR / "expected_hashes.json"


class TestPhase7RegressionMatrix(unittest.TestCase):
    def test_regression_hashes_and_no_command_errors(self) -> None:
        scenarios = sorted(REGRESSION_DIR.glob("[0-9][0-9]_*.json"))
        self.assertGreaterEqual(len(scenarios), 5, "expected at least 5 phase 7 regression scenarios")
