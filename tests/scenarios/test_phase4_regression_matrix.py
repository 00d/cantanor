...
REGRESSION_DIR = Path("scenarios/regression_phase4")
EXPECTED_PATH = REGRESSION_DIR / "expected_hashes.json"


class TestPhase4RegressionMatrix(unittest.TestCase):
    def test_regression_hashes_and_no_command_errors(self) -> None:
        scenarios = sorted(REGRESSION_DIR.glob("[0-9][0-9]_*.json"))
        self.assertGreaterEqual(len(scenarios), 8, "expected at least 8 phase 4 regression scenarios")
