...
REGRESSION_DIR = Path("scenarios/regression_phase5")
EXPECTED_PATH = REGRESSION_DIR / "expected_hashes.json"


class TestPhase5RegressionMatrix(unittest.TestCase):
    def test_regression_hashes_and_no_command_errors(self) -> None:
        scenarios = sorted(REGRESSION_DIR.glob("[0-9][0-9]_*.json"))
        self.assertGreaterEqual(len(scenarios), 12, "expected at least 12 phase 5 regression scenarios")
