/**
 * Tween convergence + animation-gate tests for spriteManager (Phase 14 M1).
 *
 ...
 * Sprite/Texture are stubbed because the module imports them at top level
 * (the sprite-sheet code paths are dead in these tests since fixtures don't
 * set unit.spriteSheet).
 *
...
vi.mock("pixi.js", () => {
  ...
  // Sprite-sheet code paths are dead in these tests (fixtures don't set
  // unit.spriteSheet) but the module imports these symbols at top level.
  class Sprite {
...
// spriteSheetLoader is imported at module top-level. Fixtures don't set
// unit.spriteSheet so the load path is dead, but stub it anyway so an
// accidental fixture change fails loudly here instead of timing out on fetch.
