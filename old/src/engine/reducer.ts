...
import { conePoints, lineAimPoints, linePoints, radiusPoints } from "../grid/areas";
...
/**
 * Start-of-turn spatial hazard check.
 *
 * Called from the end_turn handler after advanceTurn — the newly-active unit
 * rolls a basic save for each hazard zone covering their tile. Flat damage
 * (no dice) × basic-save multiplier × resistances/immunities.
 *
 * Guard on `hazards.length` so maps with no hazards consume no RNG (keeps
 * existing replays bit-stable).
 */
...
export function applyCommand(
  ...
  if (commandType === "area_save_damage") {
    ...
    // Shape branch. Defaults to burst so pre-C1 dispatches (no shape field)
    // are unchanged. For cone/line, center_x/center_y is the AIM point (a
    // direction from the caster), not a centre — the spell emanates from the
    // actor's tile. radius_feet does double duty as the length for cone/line.
    const shape = String(command.shape ?? "burst");
    let targets: string[];
    const exclude = (uid: string) => excluded !== null && uid === excluded;

    if (shape === "cone") {
      const sizeTiles = tilesFromFeet(radiusFeet);
      const ptsSet = new Set(
        conePoints(actor.x, actor.y, centerX, centerY, sizeTiles).map(
          ([x, y]) => `${x},${y}`,
        ),
      );
      targets = Object.values(nextState.units)
        .filter((u) => unitAlive(u) && !exclude(u.unitId) && ptsSet.has(`${u.x},${u.y}`))
        .filter((u) => hasTileLineOfEffect(nextState, actor.x, actor.y, u.x, u.y))
        .map((u) => u.unitId);
    } else if (shape === "line") {
      const sizeTiles = tilesFromFeet(radiusFeet);
      const blocked = new Set(nextState.battleMap.blocked.map(([x, y]) => `${x},${y}`));
      const ptsSet = new Set<string>();
      for (const [x, y] of lineAimPoints(actor.x, actor.y, centerX, centerY, sizeTiles)) {
        if (blocked.has(`${x},${y}`)) break;
        ptsSet.add(`${x},${y}`);
      }
      targets = Object.values(nextState.units)
      ...
      // burst (default) — LoE is measured from the blast centre.
      targets = unitsWithinRadiusFeet(nextState, centerX, centerY, radiusFeet, excluded);
